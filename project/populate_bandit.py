...
from bandit.mdels import Profile, Band, Venue, Event, Request, Image
...
def populate():
	...
	leif_profile = add_profile(leif_user, "Leif's band", "Glasgow", "02392561857",
		"The coolest band ever.", "B", "www.leifos.com")
...
def add_band(profile, number_of_members, genre):
	b = Band.objects.get_or_create(profile=profile)[0]
	b.number_of_members = number_of_members
	b.genre = genre
	return b
...
def add_image(profile, path, alt):
	i=Image.objects.get_or_create(path=path)[0]
	i.profile=profile
	i.alt = alt
	return i
